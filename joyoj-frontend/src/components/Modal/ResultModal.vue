...
<template>
  <div id="ResultModal">
    <a-modal
      v-model:visible="modalStore.resultModal"
      @cancel="handleCancel"
      :closable="false"
      :footer="false"
      :mask-closable="false"
    >
      ...
      <div v-else>
        ...
        <a-result v-if="resultFlag" status="success" title="恭喜你，解答成功！">
          <template #subtitle>
            <span> 请前往浏览题目提交结果页面查看更详细的答题详情 </span>
          </template>
          <template #extra>
            <a-space>
              <a-button type="primary" @click="goQuestionSubmitView"
                >前往</a-button
              >
              <a-button type="primary" @click="modalStore.setResultModal(false)"
                >关闭</a-button
              >
            </a-space>
          </template>
