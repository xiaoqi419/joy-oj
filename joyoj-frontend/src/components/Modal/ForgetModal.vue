      ...
      <div class="box-main">
        ...
        >
          ...
          <a-form-item field="emailCode" validate-trigger="blur">
            <a-input
              :max-length="6"
              v-model="formLabelAlign.emailCode"
              placeholder="邮箱验证码"
            >
              <template #prefix>
...
const getEmailCode = async () => {
  ...
  if (formLabelAlign.userEmail) {
    ...
    const res = await VerifyControllerService.getEmailCodeUsingGet(
      formLabelAlign.userEmail
    );
    if (res.code === 20000) {
      counter.value = 60;
      // 发送验证码成功
