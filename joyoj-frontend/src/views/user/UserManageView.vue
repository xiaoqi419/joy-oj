    ...
    <a-table :data="dataList" :loading="loading" @page-change="onPageChange" bordered stripe
      ...
      <template #columns>
        ...
        <a-table-column title="" align="center">
          <template #cell="{ record }">
            <a-space>
              <a-button type="primary" @click="showModal(record)">管理</a-button>
            </a-space>
          </template>
        </a-table-column>
      </template>
      ...
      <div>
        <a-form :model="form" @submit="handleSubmit" :rules="rules">
          ...
          <a-form-item field="userEmail" label="邮箱">
            <a-input v-model="form.userEmail" placeholder="please enter your email..."/>
          </a-form-item>
          <a-form-item field="userRole" label="角色">
            <a-select v-model="form.userRole" :style="{width:'160px'}" placeholder="Select"
          ...
          <a-form-item content-flex>
            <div style="width: 100%;justify-content: right;display: flex">
              ...
              <a-space>
                ...
                <a-button html-type="submit" :loading="btnLoading" type="primary" size="large">修改</a-button>
              </a-space>
