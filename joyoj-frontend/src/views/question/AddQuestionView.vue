    ...
    <a-form :model="form" :rules="rules"
      ...
      <a-form-item label="判题配置" :content-flex="false" :merge-props="false">
        <a-space direction="vertical" style="min-width: 480px;max-width: 640px">
          ...
          <a-form-item field="judgeConfig.memoryLimit" label="堆栈限制">
            <a-input-number v-model="form.judgeConfig!.memoryLimit" placeholder="请输入堆栈限制" mode="button"
                            size="large" :min="0"/>
          </a-form-item>
      ...
      <a-form-item label="测试用例配置" :content-flex="false" :merge-props="false">
        <a-form-item v-for="(judgeCaseItem,index) of form.judgeCase" :key="index" no-style>
          <a-space direction="vertical" style="min-width: 480px;max-width: 640px">
            <a-form-item :field="`form.judgeCase[${index}].input`"
                         :label="`输入用例-${index}`" style="min-width: 640px">
              <a-input v-model="judgeCaseItem.input" placeholder="请输入输入用例"/>
            </a-form-item>
            <a-form-item :field="`form.judgeCase[${index}].output`"
                         :label="`输出用例-${index}`">
              <a-input v-model="judgeCaseItem.output" placeholder="请输入输出用例"/>
            </a-form-item>
            <a-button @click="handleDelete(index)" status='danger' class="ml-12" style="margin-bottom: 18px">
              删除
            </a-button>
          </a-space>
        </a-form-item>
        <div>
          <a-button @click="handleAdd" type="outline" status="success" class="ml-12">新增测试用例</a-button>
        </div>
      </a-form-item>
